...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - あなたの曲、「 **%s** 」が %s で流れ始めましたよ！' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = '現在、 %s で「 **%s** 」が流れています。' % (
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        search_msg = await self.send_message(channel, "ビデオをさがしているよ…")
        await self.send_typing(channel)
        ...
        if not info:
            return Response("ビデオが見つからなかったんじゃあ…", delete_after=30)
        ...
        for e in info['entries']:
            ...
            if not response_message:
                ...
                return Response("わかった。気にしないでいいよ。", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response("了解！音楽が流れるまでしばらく待ってね！", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "現在流れている曲は「 **%s** 」で、「 **%s** 」さんが追加しました。%s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "現在流れている曲は「 **%s** 」です。%s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                '曲がキューにないべ！ 追加するなら {}playを実行だな！'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('オイオイ！ボイスチャンネル入ってないやん！どこ入ればいいのかわかんねえよ！')
        ...
        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    ...
                    return Response("次に流す予定の「 %s 」という曲はいまダウンロード中です…。ちょっと待ってね！" % player.playlist.peek().title)
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response('今の音量は「 `%s%%` 」でし。' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('{} は数字じゃないみたいなの…整数でおねがいね？'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('音量を「 %d 」から「 %d 」にしたよ！' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("現在流れている曲は「 **%s** 」で、「 **%s** 」さんが追加しました。%s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("現在流れている曲は「 **%s** 」です。%s\n" % (player.current_entry.title, prog_str))
